...
## __*The total number of missing values in the dataset is `r missing`.*__
...
## __*- the median number of steps taken daily remains the same at `r smed2`*__
## __*- but the mean number of steps taken daily has increased slightly from
 `r round(savg,0)` to `r round(savg2,0)`*__ 
...
|                            | **Weekdays** | **Weekends** |
|:--------------------------:|:---------:|:---------:|
| **maximum average steps taken** | **`r round(mavg2[1],0)`** | **`r round(mavg2[2],0)`** |
| **at time interval of** | **`r substr(as.character(intvl[1]),12,16)`** | **`r substr(as.character(intvl[2]),12,16)`** |
